...
export default function AddBikePage() {
  const [errorAddBike, setErrorAddBike] = useState<Record<string, string>>({});
  ...
  const addBikeMutation = useMutation({
    ...
    onSuccess: (res) => {
      console.log("✅ Add Bike success:", res.data);
      alert("Berhasil menambahkan unit Bike baru");
      router.push("/dashboard");
    },
    onError: (error) => {
      console.log("❌ Add Bike Failed:", error);
      const newError: Record<string, string> = {};
      if (axios.isAxiosError(error)) {
        const field = error.response?.data.errorType as string;
        newError[field] = error.response?.data.message as string;
        setErrorAddBike(newError);
      } else {
  ...
  function handleAddBike(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const addBikeForm = new FormData(e.currentTarget);
    const plateNumber =
      addBikeForm.get("PlateCity") +
      " " +
      addBikeForm.get("PlateNumber") +
      " " +
      addBikeForm.get("PlateArea");

    const addBikeData = {
      name: addBikeForm.get("VehicleName"),
      brand: addBikeForm.get("VehicleBrand"),
      plateNumber: plateNumber,
      year: Number(addBikeForm.get("VehicleYear")),
      price: {
        perHour: Number(addBikeForm.get("PriceOneHour")),
        perDay: Number(addBikeForm.get("PriceOneDay")),
        perWeek: Number(addBikeForm.get("PriceOneWeek")),
        perMonth: Number(addBikeForm.get("PriceOneMonth")),
      },
      vehicleStatus: addBikeForm.get("VehicleStatus"),
      vehicleType: "Bike",
    };

    // console.log(addBikeData);

    const validatedAddBikeData = unitVehicle.safeParse(addBikeData);
    if (!validatedAddBikeData.success) {
      validatedAddBikeData.error;

      const newErrors: Record<string, string> = {};

      validatedAddBikeData.error?.issues.forEach((issue) => {
        const field = issue.path[0] as string;
        newErrors[field] = issue.message;
      });

      setErrorAddBike(newErrors);
    } else {
      validatedAddBikeData.success;
      setErrorAddBike({});
      addBikeMutation.mutate(validatedAddBikeData.data);
    }
  }
    ...
    <div className="w-full">
      ...
      <div className="py-20">
        <div className="flex items-center justify-center">
          <div className="text-xl font-bold my-4">Form Tambah Unit Motor</div>
        </div>
        <div className="w-full">
          <form action="" method="post" onSubmit={handleAddBike}>
            <div className={pageCounter === 1 ? "block" : "hidden"}>
              <VehicleData errorAddBike={errorAddBike} />
            </div>
            <div className={pageCounter === 2 ? "block" : "hidden"}>
              <VehiclePrice errorAddBike={errorAddBike} />
            </div>
            {pageCounter === 1 ? (
...
type VehicleDataProps = {
  errorAddBike: Record<string, string>;
};

function VehicleData({ errorAddBike }: VehicleDataProps) {
  return (
    <div className="flex flex-col justify-center">
      ...
      <div className="flex flex-col justify-center px-8">
        ...
        <div className="text-red-500 mt-0 mb-1 text-xs">
          {errorAddBike.name ?? <p>{errorAddBike.name}</p>}
        </div>
        ...
        />
        <div className="text-red-500 mt-0 mb-1 text-xs">
          {errorAddBike.brand ?? <p>{errorAddBike.brand}</p>}
        </div>
        ...
        <div className="text-red-500 mt-0 mb-1 text-xs">
          {errorAddBike.plateNumber ?? <p>{errorAddBike.plateNumber}</p>}
        </div>
        ...
        />
        <div className="text-red-500 mt-0 mb-1 text-xs">
          {errorAddBike.year ?? <p>{errorAddBike.year}</p>}
        </div>
        <label className="mt-2 mb-1" htmlFor="VehicleYear">
          Status Kendaraan
        </label>
        ...
        <div className="text-red-500 mt-0 mb-1 text-xs">
          {errorAddBike.vehicleStatus ?? <p>{errorAddBike.vehicleStatus}</p>}
        </div>
      </div>
    </div>
  );
...
function VehiclePrice({ errorAddBike }: VehicleDataProps) {
  return (
    <div className="flex flex-col justify-center">
      <div className="px-4 mt-3 mb-1">
      ...
      <div className="flex flex-col justify-center px-8">
        ...
        />
        <div className="text-red-500 mt-0 mb-1 text-xs">
          {errorAddBike.price ?? <p>{errorAddBike.price}</p>}
        </div>
      </div>
    </div>
  );
